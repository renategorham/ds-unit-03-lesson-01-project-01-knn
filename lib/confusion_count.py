...
def confusion_matrix(csv, true_values, predicted_values, positive_class, negative_class):
    ...
    df['vote_pos'] = np.where(df['train_class'] ==pos, 1, 0)
    df['vote_neg'] = np.where(df['train_class'] == neg, 1, 0)
    ...
    df['true_pos'] = np.where(df['test_class'] == pos, np.where(df['result_by_vote'] == pos, 1, 0),0)
    df['true_neg'] = np.where(df['test_class'] == neg, np.where(df['result_by_vote'] == neg, 1, 0),0)
    df['false_pos'] = np.where(df['test_class'] == neg, np.where(df['result_by_vote'] == pos, 1, 0),0)
    df['false_neg'] = np.where(df['test_class'] == pos, np.where(df['result_by_vote'] == neg, 1, 0),0)
